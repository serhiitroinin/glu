...
- They are stored in the **macOS Keychain** (service `glu`), never written to disk in
  plaintext. Non-secret config lives in `~/.config/glu/` with `0600` permissions.
